...
const t1 = async () => {
  ...
  console.log(
    JSON.stringify(check) === JSON.stringify(items) ? 'all ok' : 'failed',
  );

  // node 22 + for this to work.
  const brandNew = await Array.fromAsync(new Chunker({ fetcher }).iterator);
  console.log(
    JSON.stringify(brandNew) === JSON.stringify(items)
      ? 'fromasync ok'
      : 'fromasync failed',
  );
};
...
const breweries = async () => {
  ...
  const getUri = ({ page, limit }) =>
    `https://api.openbrewerydb.org/v1/breweries?per_page=${limit}&page=${page + 1}`;
  ...
  const fetcher = async ({ stats }) => {
    const values = await got(
      getUri({ page: stats.fetches, limit: chunkSize }),
    ).json();
  ...
  const whiteHouse = { lat: 38.897957, lng: -77.03656, name: 'the Whitehouse' };
  const target = whiteHouse;
  const nearest = Array.from({ length: 10 }).fill({
    brewery: null,
    distance: Infinity,
  });
  ...
  console.log(
    `List of ${nearest.length} nearest breweries to ${target.name}\n` +
      nearest
  ...
  for await (const brewery of chunker.iterator) {
    if (brewery.latitude && brewery.longitude) {
      ...
      if (distance <= targetDistance)
        bulker.pusher({ values: [{ brewery, distance }] });
    }
  }
